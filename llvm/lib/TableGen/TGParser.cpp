...
Init *TGParser::ParseSimpleValue(Record *CurRec, RecTy *ItemType,
  ...
  case tgtok::Id: {
    ...
    auto NewRecOwner = llvm_ks::make_unique<Record>(GetNewAnonymousName(), NameLoc,
  ...
  case tgtok::l_paren: {         // Value ::= '(' IDValue DagArgList ')'
    ...
    std::vector<std::pair<llvm_ks::Init*, std::string> > DagArgs;
    if (Lex.getCode() != tgtok::r_paren) {
...
std::vector<std::pair<llvm_ks::Init*, std::string> >
TGParser::ParseDagArgList(Record *CurRec) {
  std::vector<std::pair<llvm_ks::Init*, std::string> > Result;
  ...
  while (1) {
    ...
    if (Lex.getCode() == tgtok::VarName) {
      ...
      if (!Val)
        return std::vector<std::pair<llvm_ks::Init*, std::string> >();
...
bool TGParser::ParseDef(MultiClass *CurMultiClass) {
  ...
  if (Name)
    ...
    CurRecOwner = llvm_ks::make_unique<Record>(GetNewAnonymousName(), DefLoc,
                                            Records, /*IsAnonymous=*/true);
...
bool TGParser::ParseClass() {
  ...
  if (CurRec) {
    ...
    auto NewRec =
        llvm_ks::make_unique<Record>(Lex.getCurStrVal(), Lex.getLoc(), Records);
    CurRec = NewRec.get();
...
bool TGParser::ParseMultiClass() {
  ...
  auto Result =
    MultiClasses.insert(std::make_pair(Name,
                    llvm_ks::make_unique<MultiClass>(Name, Lex.getLoc(),Records)));

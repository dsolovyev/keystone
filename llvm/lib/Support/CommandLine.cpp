...
OptionCategory llvm_ks::cl::GeneralCategory("General options");
...
ManagedStatic<SubCommand> llvm_ks::cl::TopLevelSubCommand;

// A special subcommand that can be used to put an option into all subcommands.
ManagedStatic<SubCommand> llvm_ks::cl::AllSubCommands;
...
void cl::ParseEnvironmentOptions(const char *progName, const char *envVar,
...
#ifdef _WIN32
  std::wstring wenvVar;
  if (!llvm_ks::ConvertUTF8toWide(envVar, wenvVar)) {
    assert(false &&
  ...
  if (!llvm_ks::convertWideToUTF8(wenvValue, envValueBuffer)) {
    assert(false &&
...
void LLVMParseCommandLineOptions(int argc, const char *const *argv,
                                 const char *Overview) {
  llvm_ks::cl::ParseCommandLineOptions(argc, argv, Overview, true);
}

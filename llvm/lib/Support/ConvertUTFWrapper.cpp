...
bool ConvertUTF8toWide(unsigned WideCharWidth, llvm_ks::StringRef Source,
...
bool convertUTF16ToUTF8String(ArrayRef<char> SrcBytes, std::string &Out) {
  ...
  if (Src[0] == UNI_UTF16_BYTE_ORDER_MARK_SWAPPED) {
    ...
    for (unsigned I = 0, E = ByteSwapped.size(); I != E; ++I)
      ByteSwapped[I] = llvm_ks::sys::SwapByteOrder_16(ByteSwapped[I]);
    Src = &ByteSwapped[0];
...
bool convertUTF16ToUTF8String(ArrayRef<UTF16> Src, std::string &Out)
{
  return convertUTF16ToUTF8String(
      llvm_ks::ArrayRef<char>(reinterpret_cast<const char *>(Src.data()),
...
static inline bool ConvertUTF8toWideInternal(llvm_ks::StringRef Source,
                                             TResult &Result) {
...
bool ConvertUTF8toWide(llvm_ks::StringRef Source, std::wstring &Result) {
...
bool ConvertUTF8toWide(const char *Source, std::wstring &Result) {
  ...
  return ConvertUTF8toWide(llvm_ks::StringRef(Source), Result);
}
...
bool convertWideToUTF8(const std::wstring &Source, std::string &Result) {
  if (sizeof(wchar_t) == 1) {
    ...
    return convertUTF16ToUTF8String(
        llvm_ks::ArrayRef<UTF16>(reinterpret_cast<const UTF16 *>(Source.data()),
                              Source.size()),
        Result);

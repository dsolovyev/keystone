...
struct TrailingObjectsImpl<Align, BaseTy, TopTrailingObj, PrevTy, NextTy,
  ...
  static LLVM_CONSTEXPR bool requiresRealignment() {
    return llvm_ks::AlignOf<PrevTy>::Alignment < llvm_ks::AlignOf<NextTy>::Alignment;
  }
  ...
  getTrailingObjectsImpl(const BaseTy *Obj,
    ...
    if (requiresRealignment())
      return reinterpret_cast<const NextTy *>(
          llvm_ks::alignAddr(Ptr, llvm_ks::alignOf<NextTy>()));
    else
  ...
  static LLVM_CONSTEXPR size_t additionalSizeToAllocImpl(
    ...
    return additionalSizeToAllocImpl(
        (requiresRealignment()
             ? llvm_ks::alignTo(SizeSoFar, llvm_ks::alignOf<NextTy>())
             : SizeSoFar) +
...
public:
  ...
  template <typename... Tys> struct FixedSizeStorage {
    template <size_t... Counts> struct with_counts {
      ...
      typedef llvm_ks::AlignedCharArray<
          llvm_ks::AlignOf<BaseTy>::Alignment, Size
          > type;
    };
  };

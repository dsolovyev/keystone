...
unsigned SubtargetEmitter::FeatureKeyValues(raw_ostream &OS) {
  ...
  OS << "// Sorted (by key) array of values for CPU features.\n"
     << "extern const llvm_ks::SubtargetFeatureKV " << Target
     << "FeatureKV[] = {\n";
...
void SubtargetEmitter::FormItineraryStageString(const std::string &Name,
  ...
  for (unsigned i = 0; i < N;) {
    ...
    ItinString += ", (llvm_ks::InstrStage::ReservationKinds)" + itostr(Kind);
...
EmitStageAndOperandCycleData(raw_ostream &OS,
  ...
  // Begin stages table
  std::string StageTable = "\nextern const llvm_ks::InstrStage " + Target +
                           "Stages[] = {\n";
  StageTable += "  { 0, 0, 0, llvm_ks::InstrStage::Required }, // No itinerary\n";
  ...
  // Closing stage
  StageTable += "  { 0, 0, 0, llvm_ks::InstrStage::Required } // End stages\n";
  StageTable += "};\n";
...
EmitItineraries(raw_ostream &OS,
  ...
  for (CodeGenSchedModels::ProcIter PI = SchedModels.procModelBegin(),
    ...
    OS << "\n";
    OS << "static const llvm_ks::InstrItinerary ";
...
void SubtargetEmitter::EmitProcessorResources(const CodeGenProcModel &ProcModel,
  ...
  OS << "static const llvm_ks::MCProcResourceDesc "
...
void SubtargetEmitter::EmitSchedClassTables(SchedClassTables &SchedTables,
  ...
  OS << "\n// {ProcResourceIdx, Cycles}\n"
     << "extern const llvm_ks::MCWriteProcResEntry "
  ...
  OS << "\n// {Cycles, WriteResourceID}\n"
     << "extern const llvm_ks::MCWriteLatencyEntry "
  ...
  OS << "\n// {UseIdx, WriteResourceID, Cycles}\n"
     << "extern const llvm_ks::MCReadAdvanceEntry "
  ...
  for (CodeGenSchedModels::ProcIter PI = SchedModels.procModelBegin(),
    ...
    OS << "static const llvm_ks::MCSchedClassDesc "
...
void SubtargetEmitter::EmitProcessorModels(raw_ostream &OS) {
  ...
  for (const CodeGenProcModel &PM : SchedModels.procModels()) {
    ...
    OS << "\n";
    OS << "static const llvm_ks::MCSchedModel " << PM.ModelName << " = {\n";
...
void SubtargetEmitter::EmitProcessorLookup(raw_ostream &OS) {
  ...
  OS << "// Sorted (by key) array of itineraries for CPU subtype.\n"
     << "extern const llvm_ks::SubtargetInfoKV "
     << Target << "ProcSchedKV[] = {\n";
...
void SubtargetEmitter::run(raw_ostream &OS) {
  ...
  OS << "namespace llvm_ks {\n";
  Enumeration(OS);
  OS << "} // end namespace llvm_ks\n\n";
  ...
  OS << "namespace llvm_ks {\n";
  OS << "extern const llvm_ks::SubtargetFeatureKV " << Target << "FeatureKV[];\n";
  OS << "extern const llvm_ks::SubtargetFeatureKV " << Target << "SubTypeKV[];\n";
  OS << "extern const llvm_ks::SubtargetInfoKV " << Target << "ProcSchedKV[];\n";
  OS << "extern const llvm_ks::MCWriteProcResEntry "
     << Target << "WriteProcResTable[];\n";
  OS << "extern const llvm_ks::MCWriteLatencyEntry "
     << Target << "WriteLatencyTable[];\n";
  OS << "extern const llvm_ks::MCReadAdvanceEntry "
     << Target << "ReadAdvanceTable[];\n";

  if (SchedModels.hasItineraries()) {
    OS << "extern const llvm_ks::InstrStage " << Target << "Stages[];\n";

...
MVT::SimpleValueType llvm_ks::getValueType(Record *Rec) {
  return (MVT::SimpleValueType)Rec->getValueAsInt("Value");
}

StringRef llvm_ks::getName(MVT::SimpleValueType T) {
  switch (T) {
...
StringRef llvm_ks::getEnumName(MVT::SimpleValueType T) {
  switch (T) {
...
///
std::string llvm_ks::getQualifiedName(const Record *R) {
...
CodeGenRegBank &CodeGenTarget::getRegBank() const {
  if (!RegBank)
    RegBank = llvm_ks::make_unique<CodeGenRegBank>(Records, getHwModes());
  return *RegBank;
}
...
CodeGenSchedModels &CodeGenTarget::getSchedModels() const {
  if (!SchedModels)
    SchedModels = llvm_ks::make_unique<CodeGenSchedModels>(Records, *this);
  return *SchedModels;
}
...
void CodeGenTarget::ReadInstructions() const {
  ...
  for (unsigned i = 0, e = Insts.size(); i != e; ++i)
    Instructions[Insts[i]] = llvm_ks::make_unique<CodeGenInstruction>(Insts[i]);
}

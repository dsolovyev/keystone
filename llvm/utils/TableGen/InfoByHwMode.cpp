...
std::string llvm_ks::getModeName(unsigned Mode) {
  if (Mode == DefaultMode)
    return "*";
...
ValueTypeByHwMode::ValueTypeByHwMode(Record *R, const CodeGenHwModes &CGH) {
  ...
  for (const HwModeSelect::PairType &P : MS.Items) {
    auto I = Map.insert({P.first, MVT(llvm_ks::getValueType(P.second))});
...
std::string ValueTypeByHwMode::getMVTName(MVT T) {
  std::string N = llvm_ks::getEnumName(T.SimpleTy);
  if (N.substr(0,5) == "MVT::")
    N = N.substr(5);
  return N;
}
...
void ValueTypeByHwMode::dump() const {
  ...
  for (const auto &P : Map)
    dbgs() << "  " << P.first << " -> "
           << llvm_ks::getEnumName(P.second.SimpleTy) << '\n';
}

ValueTypeByHwMode llvm_ks::getValueTypeByHwMode(Record *Rec,
  ...
  return ValueTypeByHwMode(llvm_ks::getValueType(Rec));
}

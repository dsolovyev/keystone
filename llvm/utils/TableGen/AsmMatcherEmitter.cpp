...
void AsmMatcherInfo::buildInfo() {
  ...
  for (unsigned VC = 0; VC != VariantCount; ++VC) {
    ...
    for (const CodeGenInstruction *CGI : Target.getInstructionsByEnumValue()) {
      ...
      auto II = llvm_ks::make_unique<MatchableInfo>(*CGI);
    ...
    for (unsigned i = 0, e = AllInstAliases.size(); i != e; ++i) {
      auto Alias = llvm_ks::make_unique<CodeGenInstAlias>(AllInstAliases[i],
                                                       Variant.AsmVariantNo,
                                                       Target);
      ...
      auto II = llvm_ks::make_unique<MatchableInfo>(std::move(Alias));
  ...
  for (auto &II : Matchables) {
    ...
    if (II->DefRec.is<const CodeGenInstruction*>()) {
      ...
      if (Constraint != "") {
        ...
        auto AliasII = llvm_ks::make_unique<MatchableInfo>(*II);
